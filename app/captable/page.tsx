          ...
          <table>
            <thead>
              <tr>
                ...
                {membre.is_admin && <th className="num">Valeur position</th>}
                {membre.is_admin && <th className="num">Gain / Perte</th>}
            ...
            <tbody>
              {engine.capTable.map((m) => (
                <tr key={m.membre_id}>
                  ...
                  {membre.is_admin && <td className="num">{fmtNum(m.valeur_position, 0)}</td>}
                  {membre.is_admin && (
                    <td className="num">
              ...
              <tr className="total">
                <td>TOTAL</td>
                {membre.is_admin && <td className="num">{fmtNum(t.totalCapital, 0)}</td>}
                <td className="num">{fmtNum(t.totalParts, 2)}</td>
                <td className="num">100,00 %</td>
                {membre.is_admin && <td className="num">{fmtNum(t.totalValeur, 0)}</td>}
                {membre.is_admin && <td className="num">{fmtNum(t.plusValue, 0)}</td>}
